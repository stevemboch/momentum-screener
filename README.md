...
- **Index Global (default):** a union of STOXX Europe 600, S&P 500, S&P MidCap 400, S&P SmallCap 600, Nasdaq 100, MSCI Japan, MSCI Pacific ex Japan and MSCI Emerging Markets. The UI can replace Nasdaq 100 with the broader Nasdaq Composite listing proxy. Constituents use the source ISIN when supplied; an exceptional unresolved source row retains a deterministic listing identity while it is resolved for Gettex. A title in more than one benchmark is emitted once with every membership retained.
...
The importer uses tested holdings disclosures. The holdings file decides
membership; ticker, name, sector, location and exchange are retained as
metadata. It derives the Yahoo ticker from the supplied exchange (for example
`7203.T`); OpenFIGI is not used to decide index membership.
...
# Default: iShares ISP6 (S&P SmallCap 600) holdings API
UNIVERSE_SP_SMALLCAP_600_CSV_URL=
UNIVERSE_NASDAQ_100_CSV_URL=
