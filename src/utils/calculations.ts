...
// Geeignet für alle Stocks im Xetra-Universum (nicht auf Top-50 beschränkt).
//
// Gate-Voraussetzungen:
//   - Kurs über MA200 (übergeordneter Aufwärtstrend intakt)
//   - r3m > 0 (relative Stärke der letzten 3 Monate positiv)
//
// 5 Signale → Score 0–1
export function calculatePullbackDetails(
  ...
  // Gate 1: Übergeordneter Aufwärtstrend muss intakt sein
  if (aboveMa200 !== true) return { score: null, signals: null }
  // Gate 2: Relative Stärke — letzten 3 Monate positiv (kein fallendes Messer)
  if (r3m === null || r3m < 0) return { score: null, signals: null }
  ...
  // S1: RSI + MA50-Kontext
  // Überverkauft bei intaktem MA50 = bestes Signal (Mean Reversion wahrscheinlich)
  // Überverkauft bei gebrochenem MA50 = Warnsignal (könnte weiterfallen)
  let s1 = 0
  if (rsi14 !== null) {
    if (rsi14 < 35 && aboveMa50 === true)       s1 = 1.0  // stark überverkauft, Trend intakt
    else if (rsi14 < 40 && aboveMa50 === true)   s1 = 0.7
    else if (rsi14 < 50 && aboveMa50 === true)   s1 = 0.4
    else if (rsi14 < 55 && aboveMa50 === true)   s1 = 0.2  // leichte Schwäche
    else if (rsi14 < 35 && aboveMa50 === false)  s1 = 0.3  // überverkauft aber MA50 gebrochen
    else if (rsi14 < 45 && aboveMa50 === false)  s1 = 0.1
    // rsi14 >= 55 mit gebrochenem MA50 → s1 = 0
  }
  ...
  if (rsi14 !== null) {
    ...
    if (prevRsi !== null && rsi14 > prevRsi && prevRsi < 45) {
      s2 = rsi14 - prevRsi > 3 ? 1 : 0.5
    }
  }
  ...
  if (volumes && volumes.length >= 10) {
    ...
    if (avgVol10 > 0) {
      ...
      if (volRatio < 0.6) s3 = 1
      else if (volRatio < 0.8) s3 = 0.5
    }
  }
  ...
  if (ma50 !== null && ma50 > 0) {
    ...
    else if (distToMa50 < 0 && distToMa50 > -0.02)  s4 = 0.7  // leicht drunter
  }

  // S5: Kurzfristige Stabilisierung — steigende Closes in letzten 3 Tagen
  // (nutzt Closes als Proxy, da intraday-Lows nicht verfügbar)
  let s5 = 0
  if (n >= 4) {
    const c1 = closes[n - 1]
    const c2 = closes[n - 2]
    const c3 = closes[n - 3]
    const c4 = closes[n - 4]
    if (c1 > c2 && c2 > c3)       s5 = 1    // 2 steigende Tage
    else if (c1 > c2 && c1 > c3)  s5 = 0.5  // heute höher als gestern und vorgestern
    else if (c1 > c4)              s5 = 0.3  // über 3-Tages-Tief
  }
...
export function recalculateAll(
  ...
  const withScores = instruments.map((inst) => {
    ...
    if (inst.closes && inst.closes.length > 0) {
      ...
      updated.accelAdaptiveEpsilon = accel.adaptiveEpsilon
