...
export interface ETFStats {
  isin: string
  name: string | null
  aum: number | null  // EUR millions → converted to EUR
  ter: number | null  // not available from this source
}
...
async function fetchXlsxSheet(url: string, sheetFile: string): Promise<string[][]> {
  ...
  const wsXml = await extractXml(buf, entries.get(sheetFile)!)
...
// Find the most recent monthly stats URL from the downloads page
async function findLatestStatsUrl(): Promise<string> {
  ...
  if (!pageRes.ok) throw new Error('Could not load stats page')
  const html = await pageRes.text()
  const match = html.match(/href="(\/resource\/blob\/[^"]*ETF-ETP-Statistic\.xlsx)"/)
  if (!match) throw new Error('No stats URL found')
  return 'https://www.cashmarket.deutsche-boerse.com' + match[1]
}

// Parse a sheet (ETF or ETC) and extract ISIN → {name, aum}
// Row structure: col2=name, col3=ISIN, col12=AUM(€m)
function parseStatsSheet(rows: string[][]): Map<string, { name: string, aum: number | null }> {
  const map = new Map<string, { name: string, aum: number | null }>()
  ...
  for (const row of rows) {
    ...
    // AUM is in € millions → convert to €
    const aum = isFinite(aumRaw) && aumRaw > 0 ? aumRaw * 1_000_000 : null

    map.set(isin, { name: name || null, aum })
  }
  return map
}

// Cache in module scope — shared across warm invocations
let cache: { ts: number, data: Map<string, { name: string, aum: number | null }> } | null = null
const CACHE_TTL = 6 * 60 * 60 * 1000  // 6 hours

async function getStatsMap(): Promise<Map<string, { name: string, aum: number | null }>> {
  if (cache && Date.now() - cache.ts < CACHE_TTL) return cache.data

  const url = await findLatestStatsUrl()

  // Fetch ETF sheet (sheet3) and ETC sheet (sheet4) in parallel
  ...
  const map = new Map([...parseStatsSheet(etfRows), ...parseStatsSheet(etcRows)])
  ...
  try {
    ...
    const results: ETFStats[] = isins.map(isin => {
      ...
      return {
        ...
        aum: entry?.aum ?? null,
        ter: null,  // Not available from Deutsche Börse stats
      }
    })
